...
- **Voice:** invocation mode, spoken-response preference, provider-processing consent, transcript retention, audio-device preference, and usage budget. The selected mode is a user-started hands-free conversation until ended; default to no recording before that start; keep credentials outside profile data.
